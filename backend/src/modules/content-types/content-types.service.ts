...
export const contentTypesService = {
  list(query: Record<string, unknown>) {
    return contentTypesRepository.findAll(query);
  },
  getOne(id: string) {
    return contentTypesRepository.findById(id);
  },
  ...
  async update(id: string, dto: { name?: string; description?: string; fields?: Array<{ name: string }> }) {
    if (dto.fields) {
      validateDuplicateFields(dto.fields);
    }
  ...
  async remove(id: string) {
    const removed = await contentTypesRepository.delete(id);
    if (!removed) {
      throw new ApiError(404, "NOT_FOUND", "Content type not found");
    }
    await invalidatePattern("entries:list:*");
